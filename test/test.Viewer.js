describe('Viewer', () => {
  ...
  it('should set molecule SVG and matches', () => {
    ...
    const listElement = viewer.listGroup.getElementsByClassName('list-group-item')[0]
    chai.expect(listElement).to.not.be.undefined
    chai.expect(listElement.classList.contains('active'))
  })

  const matches = [{"atom_indices": [1], "trivial_name": {"name": "Carbon"}}, {"atom_indices": [0], "trivial_name": {"name": "Hydroxy"}}]

  it('should set the matches and set active matches', () => {
    const viewer = new Viewer(document.getElementById('viewer-test'))
    viewer.setMolecules(molecules)
    let listElements = viewer.listGroup.getElementsByClassName('list-group-item')
    chai.expect(listElements.length).to.equal(1)

    viewer.setMatches(matches)
    listElements = viewer.listGroup.getElementsByClassName('list-group-item')
    chai.expect(listElements.length).to.equal(2)
    chai.expect(listElements[0].classList.contains('active'))

    viewer.setActiveListElement(listElements[1])
    chai.expect(listElements[1].classList.contains('active'))
  })
  ...
  it('should set the highlighted substructure', () => {
    ...
    chai.expect(substructureHighlights.length).to.equal(1)
  })
